...
export async function GET(
  ...
  try {
    ...
    for (let pageIdx = 0; pageIdx < totalPages; pageIdx++) {
      ...
      page.drawText(`Client: ${client.name} | Dept: ${dept.name} | Portal Proof | Page: ${pageIdx + 1} of ${totalPages}`, { x: 50, y: 780, size: 10, font, color: rgb(0.4, 0.4, 0.4) });
      ...
      for (let idx = startIdx; idx < endIdx; idx++) {
        ...
        if (hasBackSide) {
          ...
          const { frontBuffer, backBuffer, frontPdfBuffer, backPdfBuffer } = await getOrRenderCard(
            dept.portalShare.pressId,
            cardholder.id,
            dept.portalShare.templateId,
            null
          );
    ...
    return new Response(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Approval_Proof_${client.name.replace(/\s+/g, '_')}_${dept.name.replace(/\s+/g, '_')}.pdf"`,
      },
    });
  } catch (error) {
    console.error('Dept download approval PDF error:', error);
