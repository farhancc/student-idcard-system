                            ...
                            <tr key={ch.id}>
                              ...
                              {templateFields.map(tf => {
                                ...
                                if (tf.field === 'uniqueKey' || tf.type === 'id' || tf.field.toLowerCase().replace(/[^a-z0-9]/g, '').includes('id')) {
                                  const custom = ch.customFields ? (typeof ch.customFields === 'string' ? JSON.parse(ch.customFields) : ch.customFields) : {};
                                  const rawVal = (parsedCustom[tf.field] !== undefined && parsedCustom[tf.field] !== null && String(parsedCustom[tf.field]).trim() !== '') 
                                    ? parsedCustom[tf.field] 
                                    : (tf.field === 'uniqueKey' ? (ch.uniqueKey || custom.uniqueKey || custom.id || custom.unique_key) : null);
                  ...
                  <div className="portal-mobile-card-list" style={{ padding: '16px', flexDirection: 'column', gap: '12px' }}>
                    {tCardholders.map(ch => {
                      ...
                      return (
                        ...
                        >
                          {/* Top Row: Checkbox, Photo & Main Info */}
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <input
                              type="checkbox"
                              checked={selectedIds.includes(ch.id)}
                          ...
                          <div style={{ display: 'flex', gap: '8px', paddingTop: '10px', borderTop: '1px solid var(--glass-border)' }}>
                            <button 
                              ...
                              onClick={() => { setPreviewCardholder(ch); setPreviewSide('front'); }}
                            >
          ...
          >
            ...
            <div style={{
              ...
              {(() => {
                  ...
                  return (
                    <>
                      {displayFields.map(tf => {
                        ...
                        if (!val) {
                          if (tf.isName && previewCardholder.name && previewCardholder.name !== 'Cardholder') {
                            val = previewCardholder.name;
                          } else if (tf.field === 'designation') {
                            val = previewCardholder.designation || '';
                          } else if (tf.field === 'uniqueKey') {
                            const rawId = previewCardholder.uniqueKey || parsedCustom.id || parsedCustom.uniqueKey || parsedCustom.unique_key;
                ...
                return (
                  <>
                    {previewCardholder.name && previewCardholder.name !== 'Cardholder' && (
